...
## API Entegrasyonu ve Otomasyon Süreci

### Projenin API Haline Getirilmesi

Bu projede, kullanıcı segmentasyonu ve churn/upsell tahmini yapan makine öğrenimi modeli bir **REST API** olarak sunulmuştur. Modelin tüm fonksiyonları, FastAPI framework’ü ile bir web servisi şeklinde dışarıya açılmıştır.

- **API Endpoint:**  
  `/predict/`  
  Bu endpoint’e kullanıcı ve event verileri JSON formatında POST edilerek, modelden anlık tahmin ve segmentasyon sonuçları alınabilmektedir.
...
- **API Yanıtı:**  
  Kullanıcıya ait segment, churn olasılığı ve diğer tahminler JSON olarak döner.

---

### n8n ile Otomasyon ve API Kullanımı

Projede, veri akışını ve otomatik tahmin sürecini yönetmek için **n8n** otomasyon aracı kullanılmıştır. Süreç aşağıdaki gibi tasarlanmıştır:

1. **Veri Kaynağı:**  
   Kullanıcı profili ve event verileri iki ayrı Google Sheets dosyasından okunur.
2. **Veri Birleştirme:**  
   n8n üzerinde “Merge” node’u ile bu iki veri seti `user_id` üzerinden birleştirilir ve analiz için tek bir unified dataset oluşturulur.
3. **API’ye Veri Gönderimi:**  
   Birleştirilen veri, n8n’deki “HTTP Request” node’u ile toplu olarak FastAPI API’sine JSON formatında gönderilir.
4. **Tahmin ve Segmentasyon:**  
   API, gelen verileri işler ve her kullanıcı için segmentasyon ve churn/upsell tahminlerini döner.
5. **Sonuçların Saklanması:**  
   API’den dönen tahmin sonuçları tekrar n8n üzerinden Google Sheets’e kaydedilir. Böylece segmentler ve tahminler sürekli güncellenir ve izlenebilir.
...
- **Tam Otomasyon:**  
  Tüm pipeline otomatik çalışır, manuel müdahale gerektirmez.
- **Gerçek Zamanlı:**  
  Yeni veriler geldiğinde veya workflow tetiklendiğinde, anında tahmin ve segmentasyon yapılır.
- **API Esnekliği:**  
  API’ye farklı kaynaklardan (örn. başka otomasyon araçları, dashboardlar) da kolayca istek atılabilir.

Bu yapı sayesinde, modelin çıktıları hem gerçek zamanlı olarak hem de sürekli güncellenen bir şekilde izlenebilir ve iş süreçlerine entegre edilebilir.
